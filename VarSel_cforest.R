...
VarSel_cforest<-function(dat, ## Data frame containing y-variable as the first column, followed by the full set of x-variables.
                         ...
                         keep.prop = .9, ## Cutoff determining which step to use as a final variable-set. All variables included in the models with (smoothed) accuracy greater than keep.prop times the (smoothed) maximum accuracy are returned by the function. This thresholding decision comes after stepwise variable selection has completed. When stop.at is very small, this parameter becomes irrelevant.
